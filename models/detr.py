...
class DETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        features, pos = self.backbone(samples)  # 返回所有层的特征图 位置编码 若return_interm_layers为True的话

        src, mask = features[-1].decompose()  # 取特征图最后一层 特征序列 掩码
        assert mask is not None
        # hs (6, bs, 100, c) 如果dec_layers为6, 后续每一层都要做loss 监督学习
        hs = self.transformer(self.input_proj(src), mask, self.query_embed.weight, pos[-1])[0]

        outputs_class = self.class_embed(hs)  # 输出分类结果
        outputs_coord = self.bbox_embed(hs).sigmoid()  # 输出xywh
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1]}  # 取最后一层的数据
        if self.aux_loss:  # 除了最后一层的数据
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        :return:
        """
        outputs_without_aux = {k: v for k, v in outputs.items() if k != 'aux_outputs'}  # 取出最后一层输出的结果
