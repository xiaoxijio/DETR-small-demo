...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        bs, c, h, w = src.shape
        src = src.flatten(2).permute(2, 0, 1)  # (w * h, bs, c)
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1)  # (w * h, bs, c)
        query_embed = query_embed.unsqueeze(1).repeat(1, bs, 1)  # (100, bs, c) 100为一张图上预测多少个框 若num_queries=100
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src,
        ...
        for layer in self.layers:  # 这里会执行6层 若enc_layers=6
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for layer in self.layers:  # 这里会执行6层 若dec_layers=6
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.multihead_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)  # 交叉注意力
        # Implementation of Feedforward model
        self.linear1 = nn.Linear(d_model, dim_feedforward)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(dim_feedforward, d_model)
...
def build_transformer(args):
    return Transformer(
        ...
        nhead=args.nheads,  # 多注意力头数
        dim_feedforward=args.dim_feedforward,
        num_encoder_layers=args.enc_layers,  # 编码层数
        num_decoder_layers=args.dec_layers,  # 解码层数
